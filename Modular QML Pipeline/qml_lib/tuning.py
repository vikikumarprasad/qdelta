# qml_lib/tuning.py
# Purpose: Hyperparameter tuning for all tuners with safe + efficient defaults.
...
def _normalize_int_grid(val: Any) -> List[int]:
    """
    Normalize 'num_layers' style specs into explicit integer lists.
    """
...
def _grid_points_from_spec(spec, default_num=7):
    """
    Turn spec-like entries into explicit lists for GridSearch:
      - ('int', low, high[, N]) -> N evenly spaced ints (inclusive)
      - ('loguniform', low, high[, N]) -> N log-spaced floats
      - ('linspace', low, high[, N]) -> N linear-spaced floats
      - ('cat', [choices], _) -> list(choices)
      - lists/ranges are passed through
    """
...
def tune_model(args, search_space: Dict[str, Any], X_train, y_train):
    ...
    def cv_objective(params: Dict[str, Any]) -> float:
        # Normalize numpy scalars to plain Python scalars
        clean_params = {}
    ...
    if args.tuner == "grid":
        ...
        # Translate "num_layers" to concrete circuits for your QNN wrappers
        ...
        # Dense expansion (7 by default; override via --grid_points)
        ...
        # Threading backend avoids pickling issues with PennyLane/QNodes
        ...
        # Optional one-pass refinement around best (off by default)
            ...
            for name, values in grid_space.items():
                if name in bp and isinstance(bp[name], (int, float)):
                    ...
                    if v > 0 and name.lower() in {"c", "epsilon", "alpha", "sigma", "lr"}:
                        ...
                        refine_grid[name] = values  # leave unchanged
                else:
                    refine_grid[name] = values  # categorical / missing -> keep
    ...
    if args.tuner == "skopt":
        ...
        # QNN layer → encoding_circuit translation
        ...
        # Filter to estimator-accepted keys, then convert to dimensions
        ...
        # Parallel proposals (keep modest to avoid oversubscription)
        n_jobs = int(getattr(args, "n_jobs", 1))
        n_points = max(1, min(4, n_jobs // 3 or 1))

        # Initial Sobol points (power-of-two-ish and dimension-aware)
        n_init = max(8, 2 * len(sk_space) + 2)
        ...
        # Threading backend avoids PL/QNode pickling issues
        with parallel_backend("threading"):
