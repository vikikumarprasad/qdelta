...
def make_gpr(params: dict, seed: int):
    ...
    k = (
        ConstantKernel(constant_value=params["c_const"], constant_value_bounds="fixed")
...
def run_variant(variant_name: str,
    ...
    pipe_delta = Pipeline(pre_delta.steps + [("gpr", make_gpr(best_params_delta, args.seed))])
    ...
    pipe_direct = Pipeline(pre_direct.steps + [("gpr", make_gpr(best_params_direct, args.seed))])
