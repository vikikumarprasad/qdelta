# HPC_CML_KRR.py — Kernel Ridge: Δ-learning + Direct DFT, BOTH (ALL + Q9) variants in one run
# - Train-only correlation pruning (Xabier-style, |r|>0.90)
# - Optuna tuning with CV + StandardScaler in pipeline
# - Runs BOTH feature variants in one job: ALL (pruned) and Q9 (your nine descriptors)
# - Saves publication-style vector PDFs (blue dots, dashed red y=x, beige stats box)
# - Produces models, predictions, per-variant metrics, and a top-level summary CSV
#
# Usage (no SLURM change needed):
#   python HPC_CML_KRR.py --data_dir /path/to/data --output_dir /path/to/run
# Optional flags:
#   --target_col ae_delta|dh_delta   (default: ae_delta)
#   --n_trials 100                   (Optuna trials, default 100)
#   --cv_folds 5 --cv_repeats 2      (CV settings)
#   --run_variants both|all|q9       (default: both)
#   --feature_selection corr90|none  (default: corr90)
#   Extra/unknown args are ignored safely (works with your SLURM wrapper).
...
# ---------- Optuna helpers ----------
def build_krr_params(trial: opt.trial.Trial, kernel: str):
    """
    Return KRR kwargs tailored to the chosen kernel (avoid irrelevant keys).
    Use a single consistent distribution for 'gamma' wherever it appears
    to avoid Optuna distribution conflicts across trials.
    """
    params = {"alpha": trial.suggest_float("alpha", 1e-6, 1e2, log=True),
              "kernel": kernel}
    if kernel in ("rbf", "poly", "sigmoid"):
        params["gamma"]  = trial.suggest_float("gamma", 1e-5, 1e1, log=True)   # consistent name & distribution
    if kernel == "poly":
...
def tune_krr(X: pd.DataFrame, y: np.ndarray,
             n_trials: int, folds: int, repeats: int, seed: int, out_csv: Path):
    ...
    def objective(trial: opt.trial.Trial) -> float:
        ...
        params = build_krr_params(trial, kernel)
        Xv, yv = X.values, y.astype(float).ravel()

        fold_mae = []
        for i, (tr_idx, va_idx) in enumerate(rkf.split(Xv), start=1):
            pipe = make_pipeline(StandardScaler(), KernelRidge(**params))
    ...
    # Build a fully-specified param dict (handles conditional params cleanly)
    final_params = build_krr_params(opt.trial.FixedTrial(best), kernel=best["kernel"])
    return study.best_value, best, final_params
...
def run_variant(variant_name: str,
    ...
    # 2) Train-only correlation pruning (optional)
    ...
    # 4A) Tune KRR on Δ (CV), fit on all train, predict Δ, reconstruct DFT
    cv_mae_delta, raw_params_delta, krr_params_delta = tune_krr(
        X=Xtr, y=y_tr_delta, n_trials=args.n_trials,
        folds=args.cv_folds, repeats=args.cv_repeats, seed=args.seed,
        out_csv=vdir / "krr_delta_optuna_trials.csv"
    )
    with open(vdir / "krr_delta_best_params.json", "w") as f:
        json.dump(raw_params_delta, f, indent=2)

    pipe_delta = make_pipeline(StandardScaler(), KernelRidge(**krr_params_delta))
    ...
    # 4B) Tune KRR on DFT (direct), fit, predict
    cv_mae_dft, raw_params_dft, krr_params_dft = tune_krr(
        X=Xtr, y=y_tr_dft, n_trials=args.n_trials,
        folds=args.cv_folds, repeats=args.cv_repeats, seed=args.seed,
        out_csv=vdir / "krr_direct_optuna_trials.csv"
    )
    with open(vdir / "krr_direct_best_params.json", "w") as f:
        json.dump(raw_params_dft, f, indent=2)

    pipe_direct = make_pipeline(StandardScaler(), KernelRidge(**krr_params_dft))
    ...
    import joblib
    joblib.dump(pipe_delta,  vdir / "krr_delta_pipeline.joblib")
    joblib.dump(pipe_direct, vdir / "krr_direct_dft_pipeline.joblib")
    ...
    }, index=Xte.index).to_csv(vdir / "krr_predictions.csv", index=True)
    ...
    parity_plot_pdf(
        ...
        out_pdf=pdir / "krr_parity_delta_only.pdf",
        xlabel=f"Predicted {delta_label}", ylabel=f"True {delta_label}",
        title=None, mae=mae_delta, r2=None
    )
    parity_plot_pdf(
        y_true=y_te_dft, y_pred=pred_dft_from_delta,
        out_pdf=pdir / "krr_parity_dft_from_delta.pdf",
        xlabel=f"Predicted {dft_label}", ylabel=f"True {dft_label}",
        title=None, mae=mae_dft_from_delta, r2=r2_dft_from_delta
    )
    parity_plot_pdf(
        y_true=y_te_dft, y_pred=pred_dft_direct,
        out_pdf=pdir / "krr_parity_direct_dft.pdf",
        xlabel=f"Predicted {dft_label}", ylabel=f"True {dft_label}",
        title=None, mae=mae_dft_direct, r2=r2_dft_direct
    )
    ...
    **Target:** {tcol}
    ...
    ## Artifacts
    - Plots: `plots/krr_parity_delta_only.pdf`, `plots/krr_parity_dft_from_delta.pdf`, `plots/krr_parity_direct_dft.pdf`
    - Pipelines: `krr_delta_pipeline.joblib`, `krr_direct_dft_pipeline.joblib`
    - Tuning logs: `krr_*_optuna_trials.csv`, best params JSONs
    """).strip()
...
def main():
    ...
    parser.add_argument("--model", default="krr")
    ...
    # ignored legacy flags (to avoid errors if SLURM passes them)
    ...
    pd.DataFrame(summary_rows).to_csv(Path(args.output_dir) / "KRR_summary_all_vs_q9.csv", index=False)
