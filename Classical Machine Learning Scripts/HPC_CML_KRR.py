# HPC_CML_KRR.py
# Kernel Ridge Regression for delta-learning and direct DFT prediction.
...
def build_krr_params(trial: opt.trial.Trial, kernel: str):
    """
    Returns a KernelRidge kwargs dict tailored to the chosen kernel.

    Uses consistent parameter names and distributions to avoid Optuna conflicts across trials.
    """
    params = {"alpha": trial.suggest_float("alpha", 1e-6, 1e2, log=True), "kernel": kernel}
...
def tune_krr(X: pd.DataFrame, y: np.ndarray,
    ...
    """
    Runs Optuna hyperparameter search for KRR using cross-validated MAE.

    Returns the best CV MAE, the raw Optuna best params, and a fully-specified KRR param dict.
    """
...
def run_variant(variant_name: str,
                ...
                summary_rows: list):
    """
    Runs delta-learning and direct DFT KRR training for one feature variant.
