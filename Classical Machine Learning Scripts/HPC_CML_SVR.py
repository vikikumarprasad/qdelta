# HPC_CML_SVR.py — SVR Δ-learning + Direct DFT, both (ALL + Q9) variants in one run
...
#   python HPC_CML_SVR.py --data_dir /path/to/data --output_dir /path/to/run
# Optional flags:
...
def build_svr_params(trial: opt.trial.Trial):
    ...
    # Use DISTINCT names to avoid distribution conflicts
    if kernel == "rbf":
...
def run_variant(variant_name: str,
    ...
    # 4A) Tune SVR on Δ (CV), fit on all train, predict Δ, reconstruct DFT
    ...
    # 4B) Tune SVR on DFT (direct), fit, predict
