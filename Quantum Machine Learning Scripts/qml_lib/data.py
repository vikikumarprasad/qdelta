# data.py
# Loads and preprocesses train/test data for the QML pipeline.
...
def load_data(
    ...
    """
    Loads train and test CSVs, selects features, scales inputs, and returns arrays.

    Returns:
        X_train, X_test, y_train, y_test, pm7_test, dft_true
    """
    ...
    # verifies all requested features are present in both splits
    ...
    # selects target column names based on whether modeling atomization energy or enthalpy
    ...
    # selects the training label based on mode
    ...
    # locates the PM7 column in the test set with a fallback search
    if pm7_default in test_df.columns:
        ...
        if not pm7_candidates:
            raise KeyError(
                f"PM7 column not found. Expected '{pm7_default}', "
    ...
    # checks all arrays for NaN or Inf before proceeding
    ...
    # scales features to [-1, 1] using train statistics only to prevent data leakage
    scaler = MinMaxScaler(feature_range=(-1, 1))
    X_train = scaler.fit_transform(X_train_df)
    X_test  = scaler.transform(X_test_df)

    # optionally reduces dimensionality with PCA fitted on train only
    ...
    # checks that feature dimensions match the chosen re-encoding strategy
    ...
    # clips values to guard against small numeric drift after scaling
